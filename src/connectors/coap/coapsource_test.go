...
func startCoAPSource(t *testing.T, protocol sources.CoAPProtocol, addr string, method string, timeout time.Duration) (<-chan *message.RunnerMessage, func()) {
    t.Helper()
    cfg := &sources.SourceCoAPConfig{
        Protocol: protocol,
        Address:  addr,
        Path:     testPath,
        Method:   method,
...
func TestCoAPSourceUDPAckChanged(t *testing.T) {
    ...
    ch, stop := startCoAPSource(t, sources.CoAPProtocolUDP, addr, "POST", 2*time.Second)
    defer stop()
...
func TestCoAPSourceUDPTimeoutGatewayTimeout(t *testing.T) {
    ...
    ch, stop := startCoAPSource(t, sources.CoAPProtocolUDP, addr, "POST", 200*time.Millisecond)
    defer stop()
...
func TestCoAPSourceTCPAckChanged(t *testing.T) {
    ...
    ch, stop := startCoAPSource(t, sources.CoAPProtocolTCP, addr, "POST", 2*time.Second)
    defer stop()
...
func TestCoAPSourceTCPTimeoutGatewayTimeout(t *testing.T) {
    ...
    ch, stop := startCoAPSource(t, sources.CoAPProtocolTCP, addr, "POST", 200*time.Millisecond)
    defer stop()
    _ = ch
